...
io.on("connection", socket => {
  ...
  socket.on("start", async cfg => {
    try {
      ...
      if (!blocks.length) {
        return socket.emit("log", "No valid message blocks found");
      }

      let index = 0;
      const delayMs = Math.max(10, parseInt(cfg.delay) || 10) * 1000;

      socket.emit("log", `Loaded ${blocks.length} blocks | Delay ${delayMs/1000}s`);
      ...
      async function sendNext() {
        ...
        try {
          ...
          socket.emit("log", `Sent block ${index + 1}/${blocks.length}`);
