...
/* ================= SESSION PATH ================= */
/* On Render: mount persistent disk and set SESSION_PATH */
...
async function startWhatsApp() {
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    ...
    if (connection === "open") {
      ...
      // Auto-send groups to ALL connected UIs
...
io.on("connection", socket => {
  ...
  socket.on("ping", () => {
    if (isConnected) socket.emit("pong", "🟢 Connected as " + sock.user.id);
  ...
  socket.on("pair", async raw => {
    if (isConnected) {
      return socket.emit("log", "Already logged in. Pairing not required.");
    }
    try {
